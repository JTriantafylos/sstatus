...
std::vector<std::shared_ptr<StatusItem>> ConfigParser::loadStatusItemsFromConfig(const std::string& configFilePath) {
    ...
    for (int i = 0; i < statusItemsTableSize; ++i) {
        ...
        if (isValidStatusItem(statusItemsArray[i])) {
            ...
            item = std::make_shared<StatusItem>("echo \"Malformed status item configuration!\"", "#ff0000", "", "", -1);
        }
...
bool ConfigParser::isValidStatusItem(toml::node_view<toml::node> node) {
    if (node["Script"] && node["ForegroundColor"] && node["BackgroundColor"] && node["BorderColor"] &&
        node["Interval"]) {
        return true;
    } else {
        return false;
